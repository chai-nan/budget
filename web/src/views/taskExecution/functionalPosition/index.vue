...
<template>
  <div class="app-container">
    ...
    >
              ...
              <div style="display: flex; justify-content: space-between">
                <div>
                  ...
                  <span
                    @click="
                      () => {
                        (openType = 1), (openInfoShow = true);
                      }
                    "
                    style="text-decoration: underline; cursor: pointer"
                    >{{ visibleData.tj.allCount }}</span
                  >&nbsp;&nbsp;&nbsp;&nbsp; 已填报部门:<span
                    @click="
                      () => {
                        (openType = 2), (openInfoShow = true);
                      }
                    "
                    style="text-decoration: underline; cursor: pointer; color: royalblue"
                    >{{ visibleData.tj.submitCount }}</span
                  >&nbsp;&nbsp;&nbsp;&nbsp; 未填报部门:<span
                    @click="
                      () => {
                        (openType = 3), (openInfoShow = true);
                      }
                    "
                    style="text-decoration: underline; cursor: pointer; color: red"
                    >{{ visibleData.tj.notCount }}</span
                  >&nbsp;&nbsp;&nbsp;&nbsp;
                  <el-dialog
                    v-model="openInfoShow"
                    width="60%"
                    :title="
                      openType == 1
                        ? '总填报部门'
                  ...
                  >
                    <el-table
                      ...
                      :data="
                        openType == 1
                          ? visibleData.allDepts
                          : openType == 2
                          ? visibleData.submitDepts
                          : visibleData.notDepts
                      "
                    >
              ...
              <el-table-column type="expand">
                <template #default="props">
                  ...
                  >
                    ...
                    <!-- <el-table-column  width="200" align="center" label='预算表名称'  prop="budget"  ></el-table-column> -->
                    <el-table-column
                      width="120"
                      align="center"
                      label="预算费用（元）"
          ...
          >
            ...
            <div style="width: 90%; margin: 0 auto 10px" v-if="infoForm.type == 2">
              ...
              <el-form-item
                label="审核意见"
                prop="remark"
                :rules="[{ required: true, message: '请填写原因', trigger: 'blur' }]"
              >
                <el-input
                  :disabled="infoForm.type == 1"
      ...
      <!-- 批量审核弹窗 -->
      <!-- <el-dialog v-model="dialogPlshVisible" width='60%' title="批量审核" draggable  :close-on-press-escape='false'>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <!-- 未报告列表弹窗 -->
    <el-dialog
      v-model="notReportedDialogVisible"
      title="去年填报但今年未填报"
      width="60%"
      draggable
      :close-on-press-escape="false"
    >
      <el-table :data="notReportedList" height="200">
        <el-table-column align="center" label="公司名称" prop="companyName" />
        <el-table-column align="center" label="部门名称" prop="deptName" />
        <el-table-column align="center" label="预算表名称" prop="tableDisplayName" />
        <el-table-column align="center" label="备注" prop="remark">去年填报但今年未填报</el-table-column>
      </el-table> 
      
      <!-- <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="notReportedDialogVisible = false">确定</el-button>
        </span>
      </template> -->
    </el-dialog>
...
import { getDeptDataList, getNotReportedList } from "@/api/taskExecution/functionalPosition/index";
import { toReactive } from "@vueuse/core";
import { nextTick, toRaw, getCurrentInstance, ref, defineAsyncComponent } from "vue";
import businessT from "./components/business_T/index.vue";
import capitalT from "./components/capital_T/index.vue";
import { useIsopen } from "./useIsopen/index";
...
const notReportedList = ref([]);
const notReportedDialogVisible = ref(false);
...
function jumpinfo(row) {
  window.open(
    "/taskExecution/functionalPositionInfo?id=" +
      visibleData.id +
      "&itemId=" +
  ...
  // router.push('/taskExecution/functionalPositionInfo?id='+visibleData.id+'&itemId='+visibleData.leftDataList[visibleData.leftIndex].id+'&deptid='+row.deptId+'&name='+visibleData.leftTitle+'-'+visibleData.leftDataList[visibleData.leftIndex].tableDisplayName+"-"+row.deptName)
}
// 确定审核
// function plSave(){
    ...
    if (valid) {
      updateStatus(obj).then((res) => {
        ...
        getLeftList({ taskId: visibleData.id, selectType }).then((r) => {
          ...
          // getRightDataList()
          // if(visibleData.reportingType==1) businessTRef.value.changeVisibleData(visibleData.leftIndex,visibleData.leftDataList[visibleData.leftIndex].tableName,visibleData.type,visibleData.leftDataList)
          // if(visibleData.reportingType==2) businessTRef.value.changeVisibleData(visibleData.leftIndex,visibleData.leftDataList[visibleData.leftIndex].tableName,visibleData.type,visibleData.leftDataList)
          if (
            businessTRef.value &&
            businessTRef.value.changeVisibleData &&
            visibleData.reportingType == 1
...
function changeDialogVisible(row, type) {
  ...
  getNotReportedList({
    ...
    if (r.code != 200) return proxy.$message.error(r.msg);
    notReportedList.value = r.data.notReportedList || [];
    console.log("r.data.notReportedList: ", r.data.notReportedList);
    // 如果是审核操作且存在未报告列表，则显示弹窗
    if (type === 2 && notReportedList.value && notReportedList.value.length > 0) {
      notReportedDialogVisible.value = true;
    }
  ...
  getLeftList({ taskId: row.id, selectType }).then((r) => {
    ...
    // if( visibleData.leftDataList.length>0)  getRightDataList()
    // businessTRef.value.changeVisibleData(visibleData.leftIndex,visibleData.leftDataList[visibleData.leftIndex].tableName,visibleData.type,visibleData.leftDataList,visibleData.id)
  });
  switch (type) {
    case 1:
      // console.log(type)
...
function changeIndex(index, tableName, reportingType) {
  ...
  // console.log(visibleData.reportingType)
  // getRightDataList()
}
