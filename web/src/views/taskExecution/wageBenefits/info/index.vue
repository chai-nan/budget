    ...
    <div class="app-container box">
           <div class="right" style="width:100%">   
               <div style="margin-bottom:10px;display:flex;justify-content: flex-end;align-items: center">
                   <el-button   type="primary" @click="changeStatus" v-if='!visibleQueryParams.show'   v-hasPermi="['fixed:wages:update']">提交审核</el-button>
                   <el-button
                       type="warning"
                       plain
                       icon="Download"
                       @click='handleExport'
                         ...
                         v-hasPermi="['fixed:wages:export']"
                       >导出</el-button>
               </div>

               <el-table  :data="visibleData.rightDataList" height='80vh' row-key="deptId" @expand-change="handleNodeExpand"   :expand-row-keys="expandedRowKeys">
                <el-table-column type="expand">
                    <template #default="props">
                        <el-table  :data="props.row.dataTable"   v-loading="loading"  height='60vh'  @selection-change="handleSelectionChange">
                       ...
                       <el-table-column  width="110" align="center" fixed="right" label='状态'  prop="budget"  >
                             <template #default="scope">
                       ...
                       <template #default="scope">
                         <template v-if="!visibleQueryParams.show">
                           <el-button
                               link
                               type="success"
                               @click='save(scope.row,props.$index)'
                               v-if='scope.row.status==1||scope.row.status==6'
                                v-hasPermi="['fixed:wages:add']"
                               >保存</el-button>
                         </template>
                ...
                <el-table-column  width="140" align="center" label='公司名称'  prop="deptName"  >
                    <!-- <template #default="scope">
...
import { toReactive } from "@vueuse/core";
   ...
   import {getLeftList,getRightData,saveItem,updateStatus,getRightList} from "@/api/taskExecution/wageBenefits/index"
 ...
 async function handleNodeExpand(node, expandedRows){
  ...
  let r=await getLeftList({parentId:node.deptId})
    ...
    let res=await getRightData({taskId:visibleData.id,deptParentId:node.deptId,selectType,pageSize:999999})
        let arr=res.data||[]
  ...
  function formatNumber(num) {
   ...
   function save(row,index){
    visibleData.leftIndex=index
     let deptParentId=visibleData.rightDataList[visibleData.leftIndex].deptId
     let obj={
   ...
   function changeStatus(){
    if(ids.value.length==0) return proxy.$message.error("请选择提交数据")
 ...
 async  function getRightDataList(){
    ...
    let res=await getRightList({selectType:1,taskId:visibleData.id})
    res.data.forEach(item=> item['dataTable']=[])
    
    visibleData.rightDataList=res.data||[]
    if(expandedRowKeys.value.length>0){
        let r=await getLeftList({parentId:visibleData.rightDataList[visibleData.leftIndex].deptId})
        //    res.data.forEach(item=> item['show']=true)
           deptList.value=r.data
    ...
    let arr2=new Set(arr.map(item=>item.deptId))
        ...
        // visibleData.rightDataList=arr
        visibleData.rightDataList[visibleData.leftIndex].dataTable=arr||[]
        loading.value=false
    }
