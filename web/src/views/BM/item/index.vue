<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="80px">
      ...
      <el-form-item label="预算表名称" prop="tableDisplayName" label-width="100px">
        <el-input
      ...
      <el-table-column label="状态" align="center" prop="remark" >
        <template #default="scope">
                     <el-switch
                        ...
                        active-value="0"
                        inactive-value="1"
                        @change='changeItemDelFlag(scope.row.id,scope.row.delFlag)'
                     ></el-switch>
    ...
    <el-dialog :title="title" v-model="open" width="800px" append-to-body>
      <el-form ref="itemRef" :model="form" :rules="rules" label-width="80px">
        <el-row  :gutter="24">
          <el-col :span="11">
            <el-form-item label="预算表" prop="tableDisplayName">
              <el-input v-model="form.tableDisplayName" placeholder="请输入预算表"  maxlength="20"  show-word-limit/>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="费用科目" prop="subjectId">
        ...
        <el-form-item label="填报部门" prop="deptIds">
          <!-- <el-checkbox-group v-model="form.deptIds">
            <el-checkbox :label="item.id"  v-for="item in deptLeveOptions" :key='item.id'>{{item.label}}</el-checkbox>
       ...
       <el-dialog title="辅助字段管理" v-model="openZD" width="1000px" append-to-body draggable>
        ...
        <el-table  :data="zdList"  max-height='500px'>
          <el-table-column label="排序" align="center" prop="orderNum" min-width="50px" >
              <template #default="scope">
                <!-- <el-input v-model="scope.row.fieldDisplayName" placeholder="请输入字段名称" /> -->
                {{scope.row.orderNum}}
              </template>
            </el-table-column>
            ...
            <el-table-column label="是否查询" align="center" prop="fieldQuery" >
              <template #default="scope">
                          <!-- <el-switch
                              v-model="scope.row.fieldDisplay"
                              active-value="0"
                              inactive-value="1"
                          ></el-switch> -->
...
<script setup name="Item">
  import {getItem, listItem,addItem,updateItem,updateStatus,GetZdList,addZdList,delZDItem,updateZDItem} from "@/api/BM/item";
...
function getList() {
  loading.value = true;
  listItem({reportingType:reportingType.value,...queryParams.value}).then(response => {
