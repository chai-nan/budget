...
import net.cmkj.common.annotation.Excels;
import net.cmkj.common.config.CmkjConfig;
import net.cmkj.common.core.domain.AjaxResult;
import net.cmkj.common.core.text.Convert;
import net.cmkj.common.exception.UtilException;
import net.cmkj.common.utils.file.FileTypeUtils;
import net.cmkj.common.utils.file.FileUtils;
import net.cmkj.common.utils.file.ImageUtils;
import net.cmkj.common.utils.poi.ExcelHandlerAdapter;
import net.cmkj.common.utils.reflect.ReflectUtils;
...
{
    private static final Logger log = LoggerFactory.getLogger(ExcelUtilNew.class);
    ...
    public ExcelUtilNew(Class<T> clazz)
    {
        this.clazz = clazz;
    }
    ...
    public void init(Workbook workbook,List<T> list, String sheetName, String title, Excel.Type type)
    {
        if (list == null)
        {
    ...
    public void createTitle() {
        ...
        for (Object[] objects : fields) {
            ...
            if (StringUtils.isNotEmpty(attr.parentName())) {
                hasParentName = true;
                break;
            }
        }
        ...
        if (hasParentName) {
            ...
            for (Object[] objects : fields) {
                Excel attr = (Excel) objects[1];
                String parentName = attr.parentName();
                if(parentName.startsWith("number+")||parentName.startsWith("number-")){
                    String[] split = parentName.split(",");
                    if(split.length==3){
                        int num = 0;
                        if(split[0].equals("number+")){
                            num = number + Integer.valueOf(split[1]);
                        }else if(split[0].equals("number-")){
                            num = number - Integer.valueOf(split[1]);
                        }
                        parentName = split[2].replace("{}",num+"");
                    }
                }
                String name = attr.name(); // 获取二级表头名称
                if(name.startsWith("number+")||name.startsWith("number-")){
                    ...
                    if(split.length==3){
                        if(split.length==3){
                            ...
                            name = split[2].replace("{}",num+"");
                        }
                    }
                }
                CellStyle headerStyle = styles.get(StringUtils.format("header_{}_{}", attr.headerColor(), attr.headerBackgroundColor()));
                if (StringUtils.isNotEmpty(parentName)) {
                    if (lastParentName == null || !lastParentName.equals(parentName)) {
                        // 合并前一个一级表头的单元格
                        if (lastParentName != null && colIndex - startColIndex > 1) {
                            sheet.addMergedRegion(new CellRangeAddress(parentRow.getRowNum(), parentRow.getRowNum(), startColIndex, colIndex - 1));
                        }
                        startColIndex = colIndex;
                        lastParentName = parentName;

                        Cell parentCell = parentRow.createCell(colIndex);
                        parentCell.setCellValue(parentName);
                        parentCell.setCellStyle(headerStyle);
                    }
                } else {
                    // 合并没有一级表头的列
                    if (lastParentName != null) {
                        if (colIndex - startColIndex > 1) {
                            sheet.addMergedRegion(new CellRangeAddress(parentRow.getRowNum(), parentRow.getRowNum(), startColIndex, colIndex - 1));
                        }
                        lastParentName = null; // 重置
                    }
                    // 设置二级表头
                    Cell cell = parentRow.createCell(colIndex);
                    cell.setCellValue(name);
                    cell.setCellStyle(headerStyle);
                    sheet.addMergedRegion(new CellRangeAddress(parentRow.getRowNum(), parentRow.getRowNum() + 1, colIndex, colIndex));
                }
                colIndex++;
            }
            // 合并最后一个一级表头的单元格
...
//    }
    ...
    public void exportExcel(Workbook workbook, List<T> list, String sheetName,Integer number)
    {
        this.number = number;
        exportExcel2(workbook,list, sheetName, StringUtils.EMPTY);
    }
...
//     * @param response 返回数据
     ...
     * @return 结果
     */
    public void exportExcel2( Workbook workbook,List<T> list, String sheetName, String title)
    {
        //response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        //response.setCharacterEncoding("utf-8");
        this.init(workbook,list, sheetName, title, Excel.Type.EXPORT);
        //exportExcel();
        writeSheet();
    }
...
//    }
    ...
    {
        String downloadPath = CmkjConfig.getDownloadPath() + filename;
