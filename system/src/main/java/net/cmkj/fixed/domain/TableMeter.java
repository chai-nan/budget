...
/**
 * 预算填报项目【计量器】对象 t_reporting_table_meter
 * 
 * @author cmkj
 * @date 2024-08-28
 */
@Data
@TableName("t_reporting_table_meter")
public class TableMeter implements Serializable {
    @Serial
    ...
    /** 填报类型：1:安全类  2：非安全类 */
    @Excel(name = "填报类型", readConverterExp = "1=安全类,2=非安全类")
    private Integer reportingType;

    /** 项目类别：1:校准 2：维修 3：改造 */
    @Excel(name = "项目类别", readConverterExp = "1=校准,2=维修,3=改造")
    ...
    /** 上年预算（元） */
    @Excel(name = "number+,0,{}年预算（元）", cellType = Excel.ColumnType.NUMERIC)
    private BigDecimal lastBudget;

    /** 上年累计发生费用（元） */
    @Excel(name = "number+,0,{}年累计发生费用（元）", cellType = Excel.ColumnType.NUMERIC)
    ...
    /** 全年预计发生费用（元） */
    @Excel(name = "number+,0,{}年全年预计发生费用（元）", cellType = Excel.ColumnType.NUMERIC)
